...
byte request[] = {0x01,0x03,0x00,0x00,0x00,0x07};  //Modbus frame ,0x04,0x08
byte request_frame[8];
const int responseLength = 19;
byte received[responseLength];  // Number of expected bytes in the response from sensor
...
void CreateFrame()
{
  uint16_t crc = calculateModbusCRC(request, sizeof(request));
  memcpy(request_frame,request,sizeof(request));
...
void ReadModbusResponse()
{
  ...
  int index = 0;
  while((millis() - startTime < 1000) && index < responseLength)
  {
